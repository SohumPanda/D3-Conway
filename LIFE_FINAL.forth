...
200 z !                                              { stores dimension of array in variable z. when we say z from now, we mean  }
...
: update_neighbours                                  { updates entire neighbours array in one go with the number of alive         }
	toprow_neighbours                            { neigbours of every element in life_array_1                                 }
...
{ Assumes arrays have dimensions z by z. Compares 2 arrays to eachother        }
...
generations @ 1+ generations ! ;             { when doing a second run to collect data in same console       }
 					     { must reset generations to 0 again ( line 713 )                }
...
{ close_measurements } { after a run of life is completed and the graphics window is closed, run this  }
                       { word to save the measurements_file.csv so you can rename it and don't need    }
		       { close forth just to do another run of life                                    }
...
: reset_life_array_1                                 { Word to reset life_array_1 to all 0's                                      }
...
: reset_life_array_2                                 { Word to reset life_array_1 to all 0's                                      }
...
: reset_life_array32                                 { Word to reset life_array32 to all 0's                                      }
...
: reset_life_array_3                                 { Word to reset life_array_1 to all 0's                                      }
...
{ write a word to save the measurements being made each iteration of life } 
...
: Rpent_grandparent                                                  { imports an r pentomino grandparent    }
 3 2 pick 2 pick 1+ import_line                                      { methusela with the top left of the    }
 1 2 pick 1+ 2 pick import_line                                      { pattern at x,y                        }
...
: reset_neighbours                                   { Word to reset neighbours array to all 0's              }
  neighbours @ z @ square 0 fill ;
  
  
  
  
  
: reset_diff_array                                   { Word to reset the difference array                      }
 diff_array @ z @ square 0 fill ;




: reset_arrays                                       { Word to reset all arrays                                }
 reset_life_array_1
 reset_life_array_2
 reset_life_array_3
 reset_life_array32
...
: reset_life                                         { Word to reset life so run_life can be used again in the }                               
 reset_arrays                                        { console. Resets all arrays and resets generations       }
 0 generations ! ;                                   { variable. All other saved measurement variables such as }
                                                     { alive_2 or no_Born are overwritten each gen so don't    }
                                                     { need to be reset                                        }



variable lifetime                                    { Creates a variable called lifetime to store the number   }
                                                     { of generations to run life for if you want to run life   }
                                                     { in the console without the need for the graphics display } 

400 lifetime !


: run_life lifetime @ 0 do                           { run life runs life for no gens in lifetime. Runs life in }
 update_life                                         { in the console.                                          } 
 loop ; 
...
: new_dimension                                      { This word stores new dimension value in z and remakes all }
 z !                                                 { arrays to be of the dimension of the new value in z       }
 z @ make_array drop neighbours !		     { Need to put new dimension size on stack before running    }						 
 z @ 1- z-1 !                                        { word.                                                     } 
 z @ z-1 @ * z(z-1) !                                { z --------                                                }
 z @ square 1- z^2-1 !
 z @ make_array drop Life_Array_1 !
 z @ make_array drop Life_Array_2 !											 
 z @ make_array drop Life_Array_3 !				 
 z @ 2 * make_array drop Life_Array32 !													 
 z @ make_array drop diff_array ! ;			         			 
